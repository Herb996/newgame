...
> ## ⚠️ 2026-09-16 更正：当前主线是 **2D**，3D 不在计划内
>
> 用户明确：「当前全都基于2D，3D不关注」。
>
> - 唯一入口 = `Scenes/StartMenu.tscn`（开始菜单）→ 进游戏走 **`Scenes/Main.tscn`（2D 线）**。
> - 下面「架构主线」里的「唯一入口 = `Scenes/Main3D.tscn`（3D 唯一）」
>   以及「2D 入口层已删除：`Scenes/Main.tscn` / `main.gd` / `camera_controller.gd` / `fog_system.gd`」
>   **均已过期** —— 那四个文件都在、而且正是当前在跑的线。
> - `Scenes/Main3D.tscn` 及其 3D 脚本**保留未删**（编辑器里 F6 仍可跑），只是不再演进。
>
> 下面第一部分起的 3D 期正文**保留原样作历史记录**，不逐条改写；读到与上面冲突的地方，
> 以上面的更正为准。
